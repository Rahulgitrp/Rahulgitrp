<h1 align="center">Hi 👋, I'm Rahul Patil</h1>

<h3 align="center">💻 .NET Full Stack Developer | ASP.NET Core | React.js | SQL Server</h3>

<p align="center">
Passionate Software Developer from India with hands-on experience in developing Banking Software, REST APIs, and Full Stack Web Applications using Microsoft Technologies.
</p>

<p align="center">
  <a href="mailto:rahulspatil200@gmail.com">
    <img src="https://img.shields.io/badge/Email-rahulspatil200%40gmail.com-red?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>

  <a href="https://www.linkedin.com/in/rahul-patil-software">
    <img src="https://img.shields.io/badge/LinkedIn-Rahul%20Patil-blue?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
</p>

---

# 👨‍💻 About Me

- 💼 Working as a **.NET Full Stack Developer**
- 🏢 **Ifinix Fintech Pvt. Ltd.**
- 🏦 Working on **Banking Software Solutions**
- 🔧 Developing REST APIs using ASP.NET Core
- ⚛️ Building Frontend using React.js
- 🗄️ Writing SQL Queries & Stored Procedures
- 🌱 Learning **Microservices, Docker, Azure, AI Tools & Design Patterns**
- 📍 Pune, Maharashtra, India

---

# 💼 Professional Experience

## 🏢 Ifinix Fintech Pvt. Ltd.

### Software Developer Intern
📅 Apr 2025 – Jul 2025

- Banking Software Development
- API Development
- React Development
- SQL Development

### .NET Full Stack Developer
📅 Aug 2025 – Present

### Responsibilities

- Develop Banking Software
- Develop REST APIs
- Build React.js Applications
- SQL Server Database Design
- Stored Procedures
- Bug Fixing
- Performance Optimization
- Team Collaboration

---

# 🛠️ Tech Stack

## 💻 Programming Languages

<p>
  <img src="https://skillicons.dev/icons?i=cs,js" />
</p>

- C#
- JavaScript

---

## ⚙️ Backend

<p>
  <img src="https://skillicons.dev/icons?i=dotnet" />
</p>

- ASP.NET Core
- Web API
- Entity Framework Core
- LINQ

---

## 🎨 Frontend

<p>
  <img src="https://skillicons.dev/icons?i=react,html,css,bootstrap" />
</p>

- React.js
- HTML5
- CSS3
- Bootstrap

---

## 🗄️ Databases

<p>
  <img src="https://skillicons.dev/icons?i=postgres,mysql" />
</p>

- Microsoft SQL Server
- PostgreSQL
- PL/SQL
- SQL

---

## 🛠️ Tools & Technologies

<p>
  <img src="https://skillicons.dev/icons?i=git,github,visualstudio,vscode,docker,azure,postman" />
</p>

- Git
- GitHub
- Visual Studio
- VS Code
- Postman
- Docker
- Azure

---

# 🚀 Featured Project

## 🏦 Banking Management System

### Description

Developed a Banking Management System using ASP.NET Core, React.js and SQL Server for secure banking operations.

### Technologies

- ASP.NET Core
- React.js
- SQL Server

### Features

- 🔐 Secure Login
- ➕ CRUD Operations
- 🌐 REST APIs
- 🗄️ SQL Stored Procedures
- 📊 Reports
- 👥 User Management

---

# 🌱 Currently Learning

- Microservices
- Docker
- Azure Cloud
- AI Powered Development
- Design Patterns
- Data Structures & Algorithms

---

# 🎯 Career Goals

- 🚀 Become Senior .NET Full Stack Developer
- ☁️ Master Azure Cloud
- 🤖 Learn AI Integration
- 🌍 Contribute to Open Source
- 💼 Build SaaS Products
- 📈 Improve System Design Skills

---

# 📜 Certifications

- Currently Learning Modern .NET Technologies
- Continuously Improving Full Stack Development Skills

---

# 📫 Connect With Me

📧 **Email**

**rahulspatil200@gmail.com**

💼 **LinkedIn**

https://www.linkedin.com/in/rahul-patil-software

---

<h3 align="center">
⭐ Thanks for visiting my profile! ⭐
</h3>
